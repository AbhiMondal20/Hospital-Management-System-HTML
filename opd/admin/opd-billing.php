                            ...
                            <form novalidate method="POST" action="">
                                ...
                                <table class='table table-bordered'>
                                    ...
                                    <tbody id='fees_tbody'>
                                        <tr>
                                            <td colspan="1">
                                                <select class="form-select select2" name="servname[]">
                                                    <option>Select Services</option>
                                                    <?php
                                                    $sql = "SELECT * FROM servmaster";
                                                    $stmt = sqlsrv_query($conn, $sql);
                                                    if ($stmt === false) {
                                                        die (print_r(sqlsrv_errors(), true));
                                                    } else {
                                            ...
                                            <td>
                                                <select class="form-select select2" name="servrate[]">
                                                    <option>Select Price</option>
                                                    <?php
                                                    $sql = "SELECT *, ROUND(servrate, 2) AS rounded_rate FROM servmaster";
                                                    $stmt = sqlsrv_query($conn, $sql);
                                                    if ($stmt === false) {
                                                        die (print_r(sqlsrv_errors(), true));
                                                    } else {
                                                        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                                            ...
                                                            echo "<option value='$servrate'>$servrate</option>";
                                                        }
                                                    }
                                                    ?>
                                                </select>
                                            ...
                                            <td>
                                                <select class="form-select select2" name="dept[]">
...
<!-- <script>
    $(document).ready(function () {
        $("#btn-add-row").click(function () {
            var row = "<tr><td><select class='form-select select2' required name='servname[]'><?php $sql = "SELECT * FROM servmaster";
            $stmt = sqlsrv_query($conn, $sql);
            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                $servname = $row['servname'];
                echo "<option value=$servname>$servname</option>";
            } ?></select></td><td><select class='form-select select2' required name='servrate[]'><?php $sql = "SELECT *, ROUND(servrate, 2) AS rounded_rate FROM servmaster";
             $stmt = sqlsrv_query($conn, $sql);
             while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                 $servrate = number_format($row['servrate'], 2);
                 echo "<option value='$servrate'>$servrate</option>";
             } ?></select></td><td><select class='form-select select2' required name='dept[]'><?php $sql = "SELECT * FROM servmaster";
              $stmt = sqlsrv_query($conn, $sql);
              while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                  $dept = $row['dept'];
                  echo "<option value=$dept>$dept</option>";
              } ?></select></td><td><input type='button' value='x' class='btn btn-danger btn-sm btn-row-remove'> </td> </tr>";
...
<script>
    $(document).ready(function () {
        function fetchPriceAndDept(selectElement) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    row.find("[name='servrate[]']").val(response.price);
                }
            });

            $.ajax({
                url: 'load/fetch_department.php',
                method: 'POST',
                data: { service: selectedService },
                success: function (response) {
                    row.find("[name='dept[]']").val(response.department);
                }
            });
        }
        ...
        $("#btn-add-row").click(function () {
            ...
            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                $servname = $row['servname'];
                echo '<option value='.$servname.'>'.$servname.'</option>';
            } ?></select></td><td><select class='form-select select2' required name='servrate[]'><option>Select Price</option></select></td><td><select class='form-select select2' required name='dept[]'><option>Select Dept</option></select></td><td><input type='button' value='x' class='btn btn-danger btn-sm btn-row-remove'></td></tr>";
            $("#fees_tbody").append(row);
        });

        // Event listener for service dropdown change
        $("body").on("change", "select[name='servname[]']", function () {
            fetchPriceAndDept($(this));
        });
